...
Every frame here is built by hand, a dozen rows at a time, and none of them
reads ``data/raw/gmsc.parquet``. The real file is the thing the contract is
...
dataset present to run at all. So the reference numbers live in the schema
docstrings (traced to ``reports/inspection.md``) and the tests here prove that
each rule fires on exactly the violation it describes.
